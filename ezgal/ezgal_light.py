...
class ezgal_light(ezgal.ezgal):
    ...
    def _load(self,
        ...
        if not (os.path.isfile(model_file)):
            ...
            model_file = '%smodels/%s' % (self.data_dir,
    ...
    def get_observed_ml_ratios(self, zf, filters=None, zs=None, squeeze=True):
        ...
        mags = self._get_mags(
            ...
            ab=True,
            normalize=False,
            squeeze=False) - self.get_distance_moduli(
                zf, zs=zs, nfilters=len(filters),
                squeeze=False)

        # calculate masses for these redshifts
        masses = self.get_masses(zf,
                                 zs,
    ...
    def get_age(self, z1, z2, units='gyrs'):
        ...
        to_return = utils.convert_time(
            self.filters[self.filter_order[0]].get_zf_grid(z1).get_ages(z2),
            incoming='yrs',
            outgoing=units)
